...
(function() {
    ...
    function slugFromPathname() {
        ...
        return seg && categories[seg] ? seg : null;
    }

    const pathSlug = slugFromPathname();
    let categoryKey = explicitKey || params.get('category') || pathSlug || 'gold-jewellery';

    const category = categories[categoryKey] || categories['gold-jewellery'];

    const syncCanonical =
        Boolean(categories[categoryKey]) &&
        (explicitKey || params.get('category') || pathSlug);
    if (syncCanonical && typeof document !== 'undefined') {
        const canonicalUrl = 'https://bharatjewellers.ca/' + categoryKey;
        const linkEl = document.querySelector('link[rel="canonical"]');
        if (linkEl) linkEl.setAttribute('href', canonicalUrl);
